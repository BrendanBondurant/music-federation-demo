...
- `artist { name }` under `credits` hops to **A**: `Credit.artist` returns a
  bare entity ref, same as `Recording.performers`, so `name` always resolves
  in the artists subgraph.
